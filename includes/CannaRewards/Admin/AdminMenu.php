...
class AdminMenu {
    ...
    private static function get_wp_api(): WordPressApiWrapper {
        if (self::$wp === null) {
            // This is a pragmatic way to get the instance in a static context
            // without changing the DI container setup just for an admin page.
            self::$wp = CannaRewards()->get(WordPressApiWrapper::class);
        }
        return self::$wp;
    }
    ...
    public static function add_admin_menu() {
        add_menu_page('Brand Settings', 'Brand Settings', 'manage_options', self::PARENT_SLUG, [self::class, 'settings_page_html'], 'dashicons-store', 20);
        add_submenu_page(self::PARENT_SLUG, 'Brand Settings', 'Brand Settings', 'manage_options', self::PARENT_SLUG, [self::class, 'settings_page_html']);
        add_submenu_page(self::PARENT_SLUG, 'QR Code Generator', 'QR Code Generator', 'manage_options', 'canna_qr_generator', [self::class, 'qr_generator_page_html']);
    }
    ...
    public static function personality_section_callback() { echo '<p>Define the core language and feel of your rewards program to match your brand\'s voice.</p>'; }
    public static function theme_section_callback() { echo '<p>Control the PWA\'s visual appearance. Use HSL values (e.g., "222.2 47.4% 11.2%") for colors, as defined in <code>globals.css</code>. Leave fields blank to use the PWA\'s default styling.</p>'; }
